...
export default function BackupPanel() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(t("JSON 파일을 읽을 수 없습니다."));
      }
      ...
      if (!response.ok) {
        throw new Error(result?.error ?? t("복원에 실패했습니다."));
      }

      showToast(
        t("복원 완료: 자산 {assets}건 · 배당 {dividends}건", {
      ...
      setError(err instanceof Error ? err.message : t("복원에 실패했습니다."));
    } finally {
  ...
  function handleImportClick() {
    const confirmed = window.confirm(
      t("가져오기를 진행하면 현재 데이터가 백업 파일의 내용으로 완전히 대체됩니다. 계속할까요?")
    );
  ...
  return (
    <section className="rounded-2xl bg-white p-6 shadow-sm dark:bg-card-dark">
      <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{t("데이터 백업")}</p>
