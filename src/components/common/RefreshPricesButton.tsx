    ...
    async (silent: boolean) => {
      ...
      try {
        ...
        if (!silent || result.updated > 0) {
          const failNote =
            result.failed.length > 0 ? t(" · 실패 {n}건", { n: result.failed.length }) : "";
          showToast(
            result.checked === 0
              ? t("시세를 조회할 주식이 없습니다.")
              : t("시세 갱신: {n}개 업데이트{failNote}", { n: result.updated, failNote })
          );
        }
      } catch {
        if (!silent) showToast(t("시세 갱신에 실패했습니다. 잠시 후 다시 시도해 주세요."));
      } finally {
    ...
    <div className="flex items-center gap-2">
      {lastRefresh && !isRefreshing && (
        <span className="text-[11px] text-gray-400 dark:text-gray-500">
          {t("{time} 갱신", { time: formatTime(lastRefresh) })}
        </span>
      )}
      <button
        type="button"
