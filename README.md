...
- **배당 자동 분석 (입력 불필요)**: 보유 주식의 티커로 외부 배당 이력(`/api/dividends/forecast` → 야후 차트 API `events=div`, 1시간 캐시)을 조회해 자동 표시합니다 — ① **예상 연간 배당**: 최근 12개월 주당 배당 × 현재 보유 수량(세전 추정, 원화 환산), **세후 추정(원천징수 15.4% 가정)**, 포트폴리오 배당수익률, 종목별 주당 배당·예상 수령액·시가 배당률 테이블. ② **다가오는 배당**: 과거 지급 주기를 추정해 다음 배당락 예상 시기(D-day)와 금액. ③ **받은 배당 기록 제안**: 외부 이력의 최근 지급 건 중 아직 기록하지 않은 것을 찾아 **원클릭으로 기록에 추가**(이미 기록된 건은 종목명+월 기준 자동 제외). 모두 추정임을 명시하고 조회 실패 종목은 표기됩니다
