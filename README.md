...
- **소유자 이름 커스터마이즈 (설정)**: **설정** 페이지에서 각 소유자 태그의 표시 이름을 실제 이름(예: 본인→"성민", 자녀→"하윤")으로 바꿀 수 있습니다. 변경 이름은 `settings` 테이블에 저장되어 배지·필터·도넛·성과 카드 등 모든 화면에 반영되고 백업에도 함께 포함됩니다. 자녀 항목을 쓰지 않으면 비워두면 되고(기본 이름으로 표시), 자녀 태그로 자녀 명의 자산·배당도 별도 관리할 수 있습니다.
