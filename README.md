...
- **대시보드**: 총 자산 요약 카드 + 자산 구성 도넛 차트(2:1 그리드, 세그먼트/범례 호버 시 중앙에 종류별 금액·비중 표시), 총 자산 추이 차트, 보유 자산 미리보기
- **포트폴리오**: 자산 종류별 섹션으로 그룹핑된 테이블 (수량/원금/평가금액/손익 컬럼, 행 클릭 시 수정 다이얼로그, USD 자산 배지 표시)
