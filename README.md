...
src/
  app/                        # 라우트 (App Router)
    ...
    settings/page.tsx         # 설정 (데이터 백업 내보내기/가져오기)
...
- **배당**: 배당·이자 수령 기록 관리 — "배당 추가" 다이얼로그(종목명 자동완성, ₩/$ 금액, **소유자**, 지급일, 메모)로 입력하고, 올해 누적·최근 12개월·월 평균·이번 달 스탯 타일과 최근 12개월 월별 배당 막대 차트, 월별 그룹 목록(소유자 배지·삭제 가능)으로 확인합니다. 배당에도 소유자 태그가 있어 상단의 **전체/본인/배우자/공동 필터**로 통계·차트·목록·예측이 모두 해당 소유자 기준으로 재계산됩니다. 자산과 별개의 이벤트 기록이라 매도한 종목의 과거 배당도 그대로 남습니다
...
- **소유자 구분 (부부 공용 모드)**: 로그인 없이 자산마다 소유자 태그(**본인/배우자/공동**)를 붙여 분리 관리합니다. 자산 추가/수정 폼에서 소유자를 선택하고(기존 데이터는 미지정 시 공동 처리 — 마이그레이션 불필요), 대시보드/포트폴리오/분석 우측 상단의 **전체/본인/배우자/공동 필터**로 모든 화면(총자산·도넛·테이블·카테고리 손익·**총 자산 추이 및 분석 전체**)이 해당 소유자 기준으로 재계산됩니다. "전체"가 부부 합산 뷰이고, 포트폴리오 각 자산에는 소유자 배지가 표시됩니다. 필터 선택은 localStorage에 유지됩니다.
- **소유자별 히스토리 & 분석**: 일별 스냅샷에 소유자별 원금·평가금액(`byOwner`)을 함께 기록하므로, 소유자 필터를 켜면 총 자산 추이·기간 수익률·최대 낙폭·성장 차트가 그 사람 기준으로 표시됩니다(소유자별 기록이 없는 과거 구간은 현재 비중으로 추정하며 안내). 분석 페이지 "전체" 뷰에는 본인/배우자/공동의 평가금액·손익을 나란히 비교하는 **소유자별 성과** 카드가 추가됩니다.
- **소유자별 구성 도넛**: 대시보드에 본인/배우자/공동의 총자산 비중 도넛이 추가됩니다 — 소유자 필터와 무관하게 항상 전체(합산) 기준으로 "우리 집 자산에서 각자의 몫"을 보여줍니다.
...
- **백업 내보내기**: 모든 자산·배당·자산 추이(스냅샷)를 `nexus-backup-YYYY-MM-DD.json` 파일 하나로 다운로드합니다. 클라우드 드라이브 등 안전한 곳에 보관하면 데이터가 초기화돼도 복원할 수 있습니다.
- **백업 가져오기**: 내보낸 JSON을 업로드해 복원합니다. 서버에서 형식·무결성을 검증(`app`/`version`/각 레코드)한 뒤, id·생성일·소유자 등 원본 필드를 **그대로 보존**해 되살립니다.
