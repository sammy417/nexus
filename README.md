...
- **포트폴리오**: **한국주식 / 해외주식 / 채권 / 현금 / 기타** 카테고리별 섹션. 주식의 국내/해외 구분은 시장(KOSPI·KOSDAQ·KRX) → 티커 형식(6자리 숫자) → 통화 순으로 자동 판별하며 저장 스키마 변경 없이 파생됩니다. 각 섹션에 카테고리 합계, **비중 Top 5 가로 바 차트**, 그리고 카테고리 내 비중(%) 컬럼이 추가된 테이블이 표시됩니다 (행 클릭/연필로 수정, 휴지통으로 삭제, USD 자산 배지)
- **표시 통화 전환**: 대시보드/포트폴리오 우측 상단의 ₩/$ 토글로 모든 금액(요약·도넛·추이 차트·테이블)을 원화 또는 달러로 표시. 달러 모드에서는 적용 환율($1 = ₩N)을 함께 표기하며, 선택은 localStorage에 저장됩니다. 환율 조회(`/api/fx`)에 실패하면 기본값(₩1,400)으로 폴백하고 그 사실을 표시합니다.
