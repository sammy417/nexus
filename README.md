...
- **소유자 구분 (부부 공용 모드)**: 로그인 없이 자산마다 소유자 태그(**본인/배우자/공동**)를 붙여 분리 관리합니다. 자산 추가/수정 폼에서 소유자를 선택하고(기존 데이터는 미지정 시 공동 처리 — 마이그레이션 불필요), 대시보드/포트폴리오/분석 우측 상단의 **전체/본인/배우자/공동 필터**로 모든 화면(총자산·도넛·테이블·카테고리 손익)이 해당 소유자 기준으로 재계산됩니다. "전체"가 부부 합산 뷰이고, 포트폴리오 각 자산에는 소유자 배지가 표시됩니다. 필터 선택은 localStorage에 유지됩니다. (총 자산 추이 등 히스토리 차트는 아직 전체 기준으로 기록되며, 필터 시 안내 문구가 표시됩니다)
